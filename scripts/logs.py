...
def gen_round_events(testbed, run, iter_per_device=True):
    ...
    if iter_per_device:
        ...
        dev_iters = [
            gen_round_events_for_iter(it, filter_dev=d)
            for d, it in zip(testbed.devs, it_copies)
        ]

        round_events = [next(di, (None, [], [])) for di in dev_iters]
        while True:
            min_round = min(
                [re[0] for re in round_events if re[0] is not None], default=None
            )
...
def extract_rx_tx_pairs(rx_df, tx_df, transmitter, receiver, bias_corrected=True):
    ...
    rel_rx = rx_df[
        (rx_df['own_number'] == receiver) & (rx_df['rx_number'] == transmitter)
    ]
    ...
    for index, row in rel_rx.iterrows():
        ...
        poss_txs = tx_df[
            (tx_df['own_number'] == transmitter)
            & (tx_df['tx_round'] == row['rx_round'])
            & (tx_df['tx_slot'] == row['rx_slot'])
        ]
...
def calc_ci_of_sd(sd, num, alpha=0.01):
    low = np.sqrt(
        ((num - 1) * (sd**2)) / scipy.stats.chi2.ppf(1.0 - alpha / 2.0, num - 1)
    )
    up = np.sqrt(((num - 1) * (sd**2)) / scipy.stats.chi2.ppf(alpha / 2.0, num - 1))
    return (low, up)


def extract_data_slots(
    rx_df, tx_df, r, a, b, tdoa_src_dev_number, init_slot, response_slot, final_slot
):
    ...
    init_rx = first_or_none(
        rx_df[
            (rx_df['rx_round'] == r)
            & (rx_df['rx_slot'] == init_slot)
            & (rx_df['own_number'] == b)
            & (rx_df['rx_number'] == a)
        ].to_dict(orient='records')
    )

    init_rx_passive = first_or_none(
        rx_df[
            (rx_df['rx_round'] == r)
            & (rx_df['rx_slot'] == init_slot)
            & (rx_df['own_number'] == tdoa_src_dev_number)
            & (rx_df['rx_number'] == a)
        ].to_dict(orient='records')
    )
    ...
    response_rx = first_or_none(
        rx_df[
            (rx_df['rx_round'] == r)
            & (rx_df['rx_slot'] == response_slot)
            & (rx_df['own_number'] == a)
            & (rx_df['rx_number'] == b)
        ].to_dict(orient='records')
    )
    response_rx_passive = first_or_none(
        rx_df[
            (rx_df['rx_round'] == r)
            & (rx_df['rx_slot'] == response_slot)
            & (rx_df['own_number'] == tdoa_src_dev_number)
    ...
    final_rx = first_or_none(
        rx_df[
            (rx_df['rx_round'] == r)
            & (rx_df['rx_slot'] == final_slot)
            & (rx_df['own_number'] == b)
            & (rx_df['rx_number'] == a)
        ].to_dict(orient='records')
    )
    final_rx_passive = first_or_none(
        rx_df[
            (rx_df['rx_round'] == r)
            & (rx_df['rx_slot'] == final_slot)
            & (rx_df['own_number'] == tdoa_src_dev_number)
...
def extract_record(
    ...
    delay_a = dur(
        ...
        (data.get('final_tx', {}) or {}).get('tx_ts', None),
    )
    round_b = dur(
        (data.get('response_tx', {}) or {}).get('tx_ts', None),
    ...
    record['init_rd_cfo'] = ci_or_none_to_rd(
        (data.get('init_rx', {}) or {}).get('ci', None)
    )
    record['response_rd_cfo'] = ci_or_none_to_rd(
        (data.get('response_rx', {}) or {}).get('ci', None)
    )
    record['final_rd_cfo'] = ci_or_none_to_rd(
    ...
    if None not in [round_a, delay_b, delay_a, round_b]:
        ...
        if tdoa_src_dev:
            if not hasattr(testbed, 'get_dist'):
                record['tdoa'] = get_dist(
                    testbed.dev_positions[da], testbed.dev_positions[tdoa_src_dev]
                ) - get_dist(
                    testbed.dev_positions[tdoa_src_dev], testbed.dev_positions[db]
                )
            else:
                record['tdoa'] = testbed.get_dist(da, tdoa_src_dev) - testbed.get_dist(
                    tdoa_src_dev, db
                )
            record['tdoa_device'] = tdoa_src_dev_number

            passive_m_a = dur(
                (data.get('init_rx_passive', {}) or {}).get(
                    'bias_corrected_rx_ts' if bias_corrected else 'rx_ts', None
                ),
            ...
            if None not in [passive_m_a, passive_m_b]:
                ...
                record['tdoa_a_relative_drift_ds'] = (
                    record['passive_m_a'] + record['passive_m_b']
                ) / (record['round_a'] + record['delay_a'])
                record['tdoa_b_relative_drift_ds'] = (
                    record['passive_m_a'] + record['passive_m_b']
                ) / (record['round_b'] + record['delay_b'])

                record['tdoa_init_rd_cfo'] = ci_to_rd(
                    (data.get('init_rx_passive', {}) or {}).get('ci')
                )
                record['tdoa_response_rd_cfo'] = ci_to_rd(
                    (data.get('response_rx_passive', {}) or {}).get('ci')
                )
                record['tdoa_final_rd_cfo'] = ci_to_rd(
                    (data.get('final_rx_passive', {}) or {}).get('ci')
                )

                record['tdoa_est_ds'] = convert_ts_to_m(
                    0.5 * record['tdoa_a_relative_drift_ds'] * round_a
                    + 0.5 * record['tdoa_b_relative_drift_ds'] * delay_b
                    - passive_m_a
                )
                if None not in [
                    record['tdoa_final_rd_cfo'],
                    record['tdoa_response_rd_cfo'],
                ]:
                    record['tdoa_est_ss_init'] = convert_ts_to_m(
                        0.5 * record['tdoa_init_rd_cfo'] * round_a
                        + 0.5 * record['tdoa_response_rd_cfo'] * delay_b
                        - passive_m_a
                    )
                    record['tdoa_est_ss_final'] = convert_ts_to_m(
                        0.5 * record['tdoa_final_rd_cfo'] * round_a
                        + 0.5 * record['tdoa_response_rd_cfo'] * delay_b
                        - passive_m_a
                    )
                    record['tdoa_est_ss_both'] = convert_ts_to_m(
                        0.25 * record['tdoa_init_rd_cfo'] * round_a
                        + 0.25 * record['tdoa_final_rd_cfo'] * round_a
                        + 0.5 * record['tdoa_response_rd_cfo'] * delay_b
                        - passive_m_a
                    )
...
def gen_tdma_twr_records(
    testbed, run, tdoa_src_dev_number=None, bias_corrected=True, slots_per_phase=None
):
        ...
        for a_ev in first_phase_txs.to_dict(orient='records'):
            ...
            for b_ev in first_phase_txs.to_dict(orient='records'):
                ...
                if a_ev['tx_slot'] < b_ev['tx_slot']:
                    ...
                    # else we use the response in the next phase
                    response_slot = b_ev['tx_slot'] + slots_per_phase
