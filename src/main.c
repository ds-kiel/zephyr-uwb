...
enum network_event {
    ...
    RANGING_MTM_SSTWR_CFO_HASHED_SLOT,
    RANGING_MTM_CFOTWR_HASHED_SLOT,
    RANGING_MTM_TDOA_BEACON_SLOT,
};
...
#else
#define SLOT_LENGTH_TIMESTAMP_MS 70
#endif
...
// frequency g_offsets are determined through two transmissions by one node and the subsequent two
...
int have_freq_offset(struct freq_offset *offsets, int offsets_length, uint8_t initiator_id, uint8_t responder_id) {
    for(int i = 0; i < offsets_length; i++) {
        if(offsets[i].from_id == initiator_id && offsets[i].to_id == responder_id) {
            return 1;
        }
    }
...
int frequencies_from_cfos(const struct dwt_ranging_frame_info *frame_infos, int round_length,
			  int repetitions, struct freq_offset *offsets)
{
    int offset_cnt = 0;
...
// output g_measurements. Only output g_measurements in which we are either responder, initiator, or passive listener
void output_measurements(const struct measurement *g_measurements,
    int tof_count,
...
void output_measurements_rtt(const struct measurement *g_measurements,
    int tof_count, enum output_mode mode, uint64_t rtc_slot_ts, const char *type) {
    ...
    enum extraction_mode mode)
{
    ...
    if(mode == EXTRACT_LOCAL_ONLY) {
        int template_cnt = fill_twr_template_from_frames(frame_infos, node_ranging_id, conf->round_length, conf->repetitions, g_templates, EXTRACT_WITH_REJECT);
        ...
        for(int i = 0; i < ranging_id_count; i++) {
            int template_cnt = fill_twr_template_from_frames(
                frame_infos, g_ranging_ids[i], conf->round_length, conf->repetitions,
...
#if CONFIG_DWT_MTM_CALCULATE_DISTANCES
	int template_cnt = extract_round_templates(frame_infos, &conf, EXTRACT_ALL);
        int offset_cnt = calculate_frequency_offsets(g_templates, template_cnt, g_offsets);
	int meas_cnt = calculate_measurements(g_templates, template_cnt, g_offsets, offset_cnt,
					      g_measurements);

        output_measurements_rtt(g_measurements, meas_cnt, OUTPUT_TDOA | OUTPUT_TWR, next_event_time, "ds_id");
#else
        output_frame_timestamps(frame_infos, conf.round_length, conf.repetitions, next_event_time);
#endif
    }
    SET_GPIO_LOW(0);
...
void dwt_mtm_sstwr_cfo_handler(struct k_work *work)
{
#if CONFIG_RANGING_RADIO_SLEEP
    radio_api->start(ieee802154_dev);
#endif
...
#if CONFIG_DWT_MTM_CALCULATE_DISTANCES
	int template_cnt = extract_round_templates(frame_infos, &conf, EXTRACT_ALL);
        int freq_cnt = frequencies_from_cfos(frame_infos, conf.round_length, conf.repetitions, g_offsets);
        int meas_cnt = calculate_measurements(g_templates, template_cnt, g_offsets, freq_cnt, g_measurements);

        output_measurements_rtt(g_measurements, meas_cnt, OUTPUT_TDOA | OUTPUT_TWR, next_event_time, "ss-hash"); // OUTPUT_ASYNC_LOOP
#else
        output_frame_timestamps(frame_infos, conf.round_length, conf.repetitions, next_event_time);
#endif
    }
...
void dwt_mtm_cca_handler(struct k_work *work) {
    ...
    struct dwt_ranging_frame_info *frame_infos;
    /* if ( !dwt_mtm_ranging(ieee802154_dev, node_ranging_id, &conf, &frame_infos) ) { */
    /*     calculate_distances_from_frames(frame_infos, EXTRACT_LOCAL_ONLY, conf.round_length, conf.repetitions, next_event_time); */
    /* } */
...
void dwt_mtm_aloha_handler(struct k_work *work)
{
	...
	int meas_cnt = calculate_measurements(g_templates, template_cnt, g_offsets, offset_cnt,
        ...
        output_measurements_rtt(g_measurements, meas_cnt, OUTPUT_TDOA | OUTPUT_TWR, next_event_time, "ds-aloha");
#else
        output_frame_timestamps(frame_infos, conf.round_length, conf.repetitions, next_event_time);
#endif
    }
...
void dwt_mtm_hashed_handler(struct k_work *work)
{
	...
	struct mtm_ranging_config conf = {
        ...
        if ( (ret = !dwt_mtm_ranging(ieee802154_dev, node_ranging_id, &conf, &frame_infos)) ) {
            ...
            output_measurements_rtt(g_measurements, meas_cnt, OUTPUT_TDOA | OUTPUT_TWR, next_event_time, "ds-hash");
#else
            output_frame_timestamps(frame_infos, conf.round_length, conf.repetitions, next_event_time);
#endif
	}
        ...
        if( (( (end_ts - start_ts) * 1000 ) / sys_clock_hw_cycles_per_sec()) > SLOT_LENGTH_TIMESTAMP_MS ) {
            LOG_WRN("MTM Hashed output took, too long: %d ms", ( (end_ts - start_ts) * 1000 ) / sys_clock_hw_cycles_per_sec());
        }
...
K_WORK_DEFINE(dwt_mtm_sstwr_cfo_work_item, dwt_mtm_sstwr_cfo_handler);
...
void network_event_handler() {
    ...
    case RANGING_MTM_HASHED_SLOT:
        ...
        break;
    case RANGING_MTM_SSTWR_CFO_HASHED_SLOT:
        k_work_submit(&dwt_mtm_sstwr_cfo_work_item);
        break;
...
void schedule_network_next_event() {
	...
	ref_slot_start_ts = ref_now + (SYS_TICK_ROUND_LENGTH - ref_now % SYS_TICK_ROUND_LENGTH);
        ...
        if( !((ref_slot_start_ts / SYS_TICK_ROUND_LENGTH) % 8) ) {
            next_event = RANGING_GLOSSY_SLOT;
        } else {
	    switch (((ref_slot_start_ts / SYS_TICK_ROUND_LENGTH)) % 3) {
	    case 0:
		next_event = RANGING_MTM_ALOHA_SLOT;
		/* break; */
	    case 1:
		next_event = RANGING_MTM_SSTWR_CFO_HASHED_SLOT;
		/* break; */
	    case 2:
